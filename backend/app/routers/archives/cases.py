...
from fastapi import APIRouter, requests, UploadFile, File, Form, BackgroundTasks
...
from ...workers.archives.cases import create_case, get_cases, delete_cases, create_pages, add_pdf_path_to_case
...
async def upload_case_to_archives(api_key: str, file_name: str, pdf: Annotated[bytes, File()], background_tasks: BackgroundTasks):
    """
    Uploads a case to the archive.
    @param api_key: API key of the user.
    @param pdf_file: PDF file of the case.
    @return: JSONResponse
    """
    print(f"Uploading case {file_name} to archives")
    try:
        # Save the pdf file
        pdf_path = "../data/uploads/" + file_name
        ...
        try: 
            ...
            background_tasks.add_task(add_pdf_path_to_case, api_key, pdf_path)
            
        except Exception as e:
            print(f"Failed to save pdf + {str(e)}")
            return JSONResponse(content={"message": f"Failed to save pdf + {str(e)}"}, status_code=400)
    except Exception as e:
        return JSONResponse(content={"message": "error in processing case; " + str(e)}, status_code=400)
    return JSONResponse(content={"message": f"{file_name} uploaded successfully"}, status_code=200)

@router.post("/case/add")
async def add_case_to_archives(api_key: str, pdf_path: str, background_tasks: BackgroundTasks):
    """
    ...
    try:
        ...
        return JSONResponse(content={"message": "error in processing case; " + str(e)}, status_code=400)
    return JSONResponse(content={"message": f"{pdf_path} archived successfully"}, status_code=200)
