...
# UTILITY FUNCTIONS
async def add_pdf_path_to_case(api_key, pdf_path):
    try:
        ...
        if pages is [] or case_name is None:
            print(f"Failed to archive page + {res}")
            return None
        ...
        if not case_id:
            print(f"Failed to archive case + {case_id}")
            return None
        ...
        print(f"Error in processing case; {str(e)}")